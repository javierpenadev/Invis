...
- [x] **CLEAN-1. Мёртвый блок OpenVPN/VPNGate в рендерере** (~175 строк, js/app.js:577-751): `initModeSwitch`, `initOpenvpn`, `renderVpngate`, `loadVpngate`, `modeState`, `ovpnState`… Зовут 7 несуществующих IPC-каналов (`mode:set`, `vpngate:*`, `openvpn:*`), читают несуществующий сеттинг `privacyMode`, ссылаются на ~12 отсутствующих DOM-элементов. Сейчас неактивны (не вызываются из `init()`), но оживут при случайной проводке; `renderVpngate` к тому же неэкранированный innerHTML. Остаток отката v1.8.0→v1.9.0.
- [x] **CLEAN-2 → CURSOR (решение пользователя: курсор ОСТАВИТЬ и улучшить).** Полностью переписан js/cursor-fx.js (~170 строк вместо 271): кольцо-контур принимает форму контура интерактивного элемента (bounding box +5px зазор, тот же border-radius — пилюли/квадраты, чекбокс даёт 28×28), точка сжимается до 6px-метки, над текстовыми полями растягивается в «каретку», клик — «затягивание» контура. Ничего не перекрывает (рамка только СНАРУЖИ элемента), подписи не дублирует (система текстовых глифов data-cursor-text удалена, 28 мёртвых атрибутов вычищены из index.html). Производительность: один rAF, позиция только через transform, rect кэшируется (scroll/resize + раз в 20 кадров), pointer-events:none, при скрытом окне rAF замирает. prefers-reduced-motion сознательно НЕ гейтит (на машине пользователя он включён — заглушит фичу; системный курсор не скрывается). Проверено скриншот-тулзой: tools/cursor-btn.png, cursor-chk.png, cursor-tab.png. Тулзе screenshot.js добавлен режим ховера (3-й аргумент — селектор) + диагностика кольца.
- [x] **CLEAN-3. Одна проба скорости вместо двух.** main.js:150 (трей) и js/app.js:704-710 (шапка) независимо качают 1 МБ каждые 5 мин. Оставить одну (в main), рендереру — пушить результат событием. Минус фоновый трафик (для Tor это ещё и отпечаток).
- [x] **CLEAN-4. Лишние файлы/вес:** 30 МБ архивов в `downloads/` (удалять после распаковки в fetch-bins.js:64-71 или не коммитить); обрезать bin/tor (доки, tor-gencert.exe, torrc-defaults — минус ~8 МБ из 93); снежинка/webtunnel в BRIDGE_PLUGINS (configs.js:26-27) без exe — убрать маппинги, пока нет бандла.

### Упростить / доделать

- [x] **SIM-1. Обновлятор:** либо хэш/подпись (SEC-4а), либо упрощение до «открыть страницу релизов» (SEC-4б). Текущий cmd+vbs-конвейер — самый сложный и наименее проверяемый код в приложении.
- [x] **SIM-2. Блок-листы: история обновлений.** Скачиваются один раз навсегда (blocklists.js:42 — `if exists return`). TTL как у onionoo (24 ч, onionoo.js:11) или ре-скачивание при старте + показывать «обновлено <дата>».
...
- [x] **SIM-5. Синхронизировать доки** (см. P2 stale-доки) — дёшево, повышает доверие.
...
## Часть 6. Защита от дурака (аудит 12.09.2026, v1.9.6)

По итогам прохода «куда дурак нажмёт не туда без обработчика». Все 15 пунктов закрыты.

### P0 (выглядело сломанным / утечка мимо Tor)
- [x] **Вечный прелоадер.** initModules прятал #appPreloader только после успешного invoke — любой сбой оставлял «мёртвое окно». Теперь прелоадер в finally + глобальный unhandledrejection → статус-бар.
- [x] **Новый Wi-Fi/USB-адаптер = DNS-утечка.** Перехват был привязан к списку адаптеров на момент включения. Теперь вотчер раз в 60 с: новый физический адаптер → DNS на 127.0.0.1 + запись в dns-backup (восстановится при выходе) + уведомление.
- [x] **«Мосты» без строк / с транспортом без плагина — тихо без мостов.** Подсказка #bridgesHint: «⚠ мосты включены, строки пустые» / «N строк, транспорт X (в комплекте obfs4, conjure)».
- [x] **Bootstrap-резольверы.** Валидация IP:порт (IPv4/IPv6/[v6]:port, порт 1-65535); мусор — не сохраняется с сообщением; полная очистка — возвращает прежний список с уведомлением.

### P1 (сработало, но непонятно)
- [x] **«Консоль I2P» при выключенном I2P** — кнопка disabled, активируется по modules:state i2p=on.
- [x] **«Доступ из LAN» без админа** — правила брандмауэра не добавляются + явное сообщение в UI (раньше только в netmode.log).
- [x] **«Обновить» качал 130+ МБ без спроса** — GitHub-asset size → update:state/available (sizeMb) → confirm() с размером перед скачиванием.
- [x] **Двойные клики** — «Проверить» IP-выхода (busy-гард), «Обновить данные» Onionoo (busy-гард).
- [x] **Порт 9050 занят другим Tor** — [err] со словами bind/already in use → статус «Порт занят другим процессом (другой Tor/SOCKS-сервис?) — …» вместо безликого «код 1».
- [x] **query.log переживал выключение галки** — файл удаляется через 5 с после выключения (dnscrypt успевает отпустить), счётчик offset сбрасывается.

### P2 (мелочи)
- [x] **NEWNYM-спам** — дебаунс 5 с.
- [x] **«Проверить обновления» дабл-клик** — дебаунс 3 с.
- [x] **Fire-and-forget init'ы** — покрыты глобальным unhandledrejection (P0).
- [x] **Клампа автосмены IP** — «abc»→0 теперь с уведомлением ««abc» → 0 мин».
- [x] **Ручной режим без выбора** — «выбрано 0 (работает как авто)» в info-строке резольверов.

Регрессия: build strict 0 ошибок, smoke exit 0, ховер-скриншот кольца OK (tools/fool-final.png).
