 ...
 * Над кнопками шар чуть больше и прозрачнее (плавно туда-обратно),
 * над текстовыми полями растягивается в «каретку». Ховер на лейбе
 * чекбокса подсвечивает сам чекбокс. Клик — кольцо коротко «затягивается».
 *
 * Принципы (по запросу): ничего не перекрывает контент — кольцо это только
 * рамка СНАРУЖИ элемента, шар полупрозрачный; смысл подписей не дублируется —
 ...
 * окне rAF замирает сам. Тач (hover:none) — фича не запускается вовсе
 * (системный курсор не скрывается — деградации нет).
 *
...
window.CursorFx = {
    ...
    init: function () {
        ...
        const underPointer = () => {
            ...
            /* Метка чекбокса = сам чекбокс: ховер на лейбе подсвечивает квадрат
             * и держится на нём, пока курсор на лейбе или на нём самом */
            const label = el.closest('label');
            const box = label?.querySelector('input[type="checkbox"], input[type="radio"]');
            if (box) return { el: box, field: false };
        ...
        const tick = () => {
            ...
            /* над интерактивом шар чуть БОЛЬШЕ и ЗАМЕТНО ПРОЗРАЧНЕЕ (плавно
             * туда-обратно), над полем — каретка, иначе обычная точка */
            const bt = caretMode ? { w: 3.5, h: 26 }
                : target ? { w: 20, h: 20 }
                : { w: 10, h: 10 };
            const bo = !visible ? 0 : (target ? 0.3 : caretMode ? 0.85 : 1);
            b.w += (bt.w - b.w) * K_GEO;
            b.h += (bt.h - b.h) * K_GEO;
            b.o += (bo - b.o) * K_FADE;
