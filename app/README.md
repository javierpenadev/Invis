# Invis — Electron tray-приложение

Клон InviZible Pro для Windows: маленькое окно + постоянная иконка в трее,
Tor / DNSCrypt / I2Pd под управлением `lib/daemons.js`. Рендерер изолирован:
`preload.js` — единственный мост к main (allowlist IPC-каналов), у UI нет
доступа к Node.
...
```bash
npm install
npm run fetch-bins   # один раз: скачать tor/dnscrypt-proxy/i2pd в bin\ (~87 МБ, SHA-256 сверяются)
npm start
```

Сборка: `npm run dist` (NSIS) / `npm run dist:portable` (bin\ кладётся в resources).
Публикация релиза: `npm run dist` → `npm run checksums` → приложить `SHA256SUMS.txt`
к релизу (авто-обновление ставит апдейт только после сверки SHA-256).
Смоук-тест GUI без демонов: `APP_SMOKE=5000 INVIS_NO_AUTOSTART=1 npm start`.
Headless-проверка демонов (без GUI): `npm run check`.
Визуальная проверка UI (с ховером на элемент): `electron.exe tools/screenshot.js out.png 4200 "#startAllBtn"`.
...
- Клик по иконке трея — показать/спрятать окно; в меню трея: статусы модулей,
  IP выхода и скорость, «Обновить данные», открыть, «Запустить/Остановить всё»,
  галка «Запускать с Windows», выход.
...
Галки «Перехват системного DNS» и «Системный прокси» — **намерения**: остановка
модулей снимает только эффект (адаптеры/прокси возвращаются), галки остаются
включёнными и применяются снова при следующем запуске модулей.

`systemProxy: true` — системный прокси Windows (WinINET) направляется на SOCKS Tor
(`socks=127.0.0.1:9050`), localhost и частные подсети идут мимо Tor (`ProxyOverride`).
Прокси «живёт вместе с Tor»: если Tor не запущен, включение галки само стартует Tor;
при остановке Tor прокси снимается, при повторной готовности — возвращается. Прежние
настройки прокси хранятся в `proxy-backup.json`; если приложение было убито и не
успело восстановить их, застрявший прокси Invis снимается при следующем старте.
